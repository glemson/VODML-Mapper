 ...
 */
SHORTVODML2VOTableAnnotator = function(tablesManager, modelsManager,exportDATA = false)
{
...
// ---
		jQuery.each(mapperState.models,function(key,value){
			if(value.name != "vo-dml"){
				var model = newModelDeclaration(VODML, value);
			}
		});
	...
	function newModelDeclaration(vodml, model){
		...
		MODEL.setAttribute(MODEL,"name",model.name);
		MODEL.setAttribute("url",model.url);
		if(model.identifier){
			MODEL.setAttribute("ID",model.identifier);
		}
	}
	...
	function checkROLE(INSTANCE, roleType, vodmlref){
		...
		if(ROLE == null || ROLE.length == 0){
			ROLE = newElement(INSTANCE,"INSTANCE")
